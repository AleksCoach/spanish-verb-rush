...
export function groupOf(infinitive: string): Group {
  return infinitive.slice(-2) as Group
}
...
export function ruleForm(infinitive: string, person: Person): string {
  return stemOf(infinitive) + ENDINGS[groupOf(infinitive)][person]
}
...
function regular(infinitive: string, meaning: string, complements: string[]): Verb {
  ...
  return { infinitive, type: 'regular', group: groupOf(infinitive), meaning, forms, complements }
}
...
function irregular(
  ...
  return { infinitive, type: 'irregular', group: groupOf(infinitive), meaning, forms, complements, note, boss }
}
...
export const REGULAR_VERBS = VERBS.filter((v) => v.type === 'regular').map((v) => v.infinitive)
